<div class="clear"></div>
<div id="footer">
    <div class="footerbottomCenter">
        <div class="footerLeftPrint">
            <div class="realizacao">
                <p id="footer_realizacao"><p>
                <div class="logosFooter">
                    <!--<a href="<?php echo $home_footerPNUD  ?>" target="_blank"><img src="./assets/img/footer/logo_PNUD.png" alt="PNUD"/></a>
                    <a href="<?php echo $home_footerFJP  ?>" target="_blank"><img src="./assets/img/footer/logo_fundacao_ joao_pinheiro.png" style="margin-top: 30px; border-left: 2px solid #CDCDCD; border-right: 2px solid #CDCDCD; padding-right: 5px; padding-left: 5px;" alt="Fundação João Pinheiro"/></a>-->
                    <a href="<?php echo $home_footerIPEA ?>" target="_blank"><img src="./assets/img/footer/logo_ipea.png" style="margin-top: 55px;" alt="IPEA"/></a>
                </div>
            </div>
        </div>
    </div>
</div>

<script>
    $("#footer_realizacao").html(lang_mng.getString("footer_realizacao"));
</script>
