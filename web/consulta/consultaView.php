...
<script type="text/javascript">
    ...
    function changeAba(switcher, t){
        ...
        if(switcher == 1){
            $("#imgTab6").show();
            $(b).html('<img src="./assets/img/icons/brazil_gray.png">');
            $(t).html('<img src="./assets/img/icons/table_white.png">');
            $(t).tooltip('hide')
            $("#form1").show();
            $("#form2").show();
            $('#btnPrintMap').hide();
            ...
            //$("#imgTab6").hide();
            //$(b).html('<img src="./assets/img/icons/table_gray.png">');
            //$(t).html('<img src="./assets/img/icons/brazil_white.png">');
            //$("#form1").hide();
            //$("#form2").hide();
            //$('#btnPrintMap').show();
    ...
    function novaPagina(sw){
        ...
        for(var i in g){
            ...
            if(g[i].l.length > 0 || g[i].est.length > 0 || g[i].mun.length > 0 || g[i].rm.length > 0 || g[i].udh.length > 0){
                t_geral = true;
                break;
            }
        }
        if(!t_geral){
            //$("#alertTabela").html('<div class="alert alert-warnning"><button type="button" class="close" data-dismiss="alert">&times;</button><?php echo $lang_mng->getString("msg_down01"); echo $path_dir; ?></div>');
            //$("#modalDownload-body").html('<?php echo $lang_mng->getString("msg_down01"); ?>');
        ...
        if(<?php echo EXIBIR_ALERTA_DOWNLOAD ?>){
            $("#modalDownload-body").html('<?php echo $lang_mng->getString("msg_down_wait"); ?> <img src="./assets/img/loader.gif" alt="" width="16" height="16"/>');
            $("#modalDownload-footer").html('<button type="button" class="btn btn-primary" disabled="true" >Download</button>');
        }
        if(sw == 1){
            ...
            //$("#form2").submit();
            //$("#alertTabela").html('<div class="alert alert-warnning"><button type="button" class="close" data-dismiss="alert">&times;</button><?php echo 'foi indo e foi' ?></div>');
        }
    }
    ...
    function consulta(){
        ...
        $.ajax({
            type: 'post',
            url:'system/modules/tabela/util/AjaxConsultaPronta.php',
            data:{'consulta':cons},
    ...
    <div class="containerPageComponentes">
        <div class="tab-content" style="min-height: 500px">
            <div class="tab-pane active" id="tab1">
                ...
                <?php include_once('../system/modules/tabela2/view/tabelaView.php'); ?>
            </div>

            <div class="tab-pane" id="tab2">
                <!--<div style="position: absolute; top: -58px; left: 0; z-index: 999; width:100%;">-->
                <div id="container_mapa_personalizado_ms" style="top: -58px; left: 0; z-index: 999; width:100%;">
					<!--<div style="background-color: white; padding-bottom: 10px; width: 100%; float: left; clear: both; border-bottom: 1px solid #ccc;">-->
					<div style="background-color: white; padding-bottom: 58px; width: 100%; clear: both; border-bottom: 1px solid #ccc;">
						<div class="iconAtlas">
							...
							<button data-toggle="modal" class="gray_button small_bt" id="fechar_map_ms"
									data-original-title="Voltar para tabela" title="" data-placement="bottom">
								<img src="./assets/img/icons/table_gray.png">
							</button>
							<button class="gray_button big_bt" onclick="printMapEvt()" type="button">
...
<div class="modal hide modal-msg-histograma" style="z-index:99999">
