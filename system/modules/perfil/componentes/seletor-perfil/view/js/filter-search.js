...
(function($) {
    ...
    $.FilterSearch = function(options) {
        ...
        if (typeof SearchComponent === 'undefined') {
            throw new Error('Comoponente filter-search.js requer componente SearchComponent.js');
            return;
        }
    ...
    var FS = function(options) {
        ...
        var showLoading = function() {
            ...
            var img = $("<img>").attr("src", "assets/img/load.gif").css("width", "100px").css("height", "120px").css("margin-left", "200px").css("margin-top", "40px");
        ...
        var setAllItensSelecteds = function(label) {
            ...
            if ($(label).is(":checked")) {
                ...
                $('.selecteds .itens-selecteds').prepend($('<div>').addClass('item-selected').attr("data-id-filter", obj.idFilter).attr('data-id', obj.idItemSelector).attr('data-id-selector', obj.idParent).html($(label).closest("label").text()).append(a));
        ...
        var internationalization = function() {
            ...
            $("#msg-bottom-search").html(lang_mng.getString("seletor_msg_search_perfil"));
            $("#title-selecteds").html(lang_mng.getString("seletor_title_selecteds"));
            $("#button-fs").html(lang_mng.getString("seletor_btn_open_selector_perfil"));
        ...
        var plotSelectors = function(selectors) {
            ...
            $('.fs').find('.selectors').append(divBodySelectors);
        ...
        var selectItens = function(label) {
            ...
            var baseUrl = splited[0] + "/" + splited[1] + "/" + splited[2] + "/" + splited[3] + "/" + lang_mng.getString("lang_id");

            if (idSelector == 2) {
                selector = "perfil_m";
            } else if (idSelector == 4) {
                selector = "perfil_uf";
            } else if (idSelector == 6) {
                selector = "perfil_rm";
            } else if (idSelector == 5) {
                selector = "perfil_udh";
            }

            baseUrl += "/" + selector;

            window.location = baseUrl + "/" + idItem;
            //alert(baseUrl);
...
//				var a = $('<a>').addClass('remove-fs-item-selected').attr("data-toggle","tooltip").attr("data-original-title",lang_mng.getString("seletor_remover_selecionado")).append($('<img>').attr('src',"assets/img/icons/deleteSelecionado.jpg"));
//				$('.selecteds .itens-selecteds').prepend($('<div>').addClass('item-selected').attr('data-id',idItem).attr('data-id-selector',idSelector).html($(label).text()).append(a));
//				$(".remove-fs-item-selected").tooltip({placement: 'top'});
//			}
//			else{											
//				pullSelectedItens(idItem, idSelector);
        ...
        var constructAccordion = function(arrayFilters, data) {
            ...
            if (typeof arrayFilters != "undefined" && arrayFilters != false) {
                $.each(arrayFilters, function(i, item) {
                    ...
                    if (selector.id != 3)//@# Tirando do Componente o FILTRO POR REGIONAIS
                        if (i == 0) {
...
//			}			
        ...
        var getTemplateAccordion = function() {
            accordion = "";
            $.ajax({
                url: "system/modules/perfil/componentes/seletor-perfil/view/accordion.html",
                async: false
            }).done(function(data) {
                accordion = $(data);
            });
