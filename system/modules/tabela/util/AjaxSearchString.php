<?php
    ...
    require_once 'protect_sql_injection.php';
    ...
    if($c > 0){
        ...
        switch ($tabela) {
            case TABELA_MUNICIPIO:
                ...
                break;
            case TABELA_REGIAOMETROPOLITANA:
//                $SQL2 =  "SELECT rm.nome, estado.uf, rm.id FROM rm
//                    INNER JOIN estado ON (estado.id = rm.fk_estado)
//                         WHERE sem_acento(rm.nome) ILIKE '%$stringTratada%' and rm.id not in ($conjunto) ORDER BY rm.nome LIMIT $c";
                $SQL2 =  "SELECT rm.nome as nome, 0 as uf, rm.id FROM rm
                         WHERE sem_acento(rm.nome) ILIKE '%$stringTratada%' and rm.id not in ($conjunto) AND rm.ativo = TRUE ORDER BY rm.nome LIMIT $c";
                break;
            case TABELA_ESTADO:
                $SQL2 = "SELECT nome, estado.uf, id FROM estado
            ...
            case TABELA_REGIAO:
                ...
                break;
            case TABELA_UDH:
                $SQL2 = "SELECT udh.nome, estado.uf, udh.id FROM udh
                    INNER JOIN municipio ON (municipio.id = udh.fk_municipio)
                 INNER JOIN estado ON (estado.id = municipio.fk_estado)
                     WHERE sem_acento(udh.nome) ILIKE '%$stringTratada%' and udh.id not in ($conjunto) ORDER BY udh.nome LIMIT $c";
                break;
            case TABELA_MUNICIPIO_ESTADO:
             $SQL2 = "SELECT nome,estado.uf,id FROM estado
