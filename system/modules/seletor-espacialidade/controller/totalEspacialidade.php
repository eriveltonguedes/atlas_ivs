<?php
    ...
    function iGetSQLSecundarioCC($Array){
            ...
            foreach($Array as $val)
                    if($val['e']==2){
						if(strlen($val['l'][0])) $muns = implode(",", $val['l']);
						if(strlen($val['est'][0])) $muns_e = implode(",", $val['est']);
						if(strlen($val['rm'][0])) $muns_r = implode(",", $val['rm']);
					}elseif($val['e']==3){
						if(strlen($val['l'][0])) $regis = implode(",", $val['l']);
						if(strlen($val['mun'][0])) $regis_m = implode(",", $val['mun']);
                    }elseif($val['e']==4){
						if(strlen($val['l'][0])) $ests = implode(",", $val['l']);
                    }elseif($val['e']==5){
						if(strlen($val['l'][0])) $udhs = implode(",", $val['l']);
						if(strlen($val['rm'][0])) $udhs_r = implode(",", $val['rm']);
						if(strlen($val['mun'][0])) $udhs_m = implode(",", $val['mun']);
                    }elseif($val['e']==6){
						if(strlen($val['l'][0])) $rms = implode(",", $val['l']);
                    }elseif($val['e']==7){
						if(strlen($val['l'][0])) $ris = implode(",", $val['l']);
                    }elseif($val['e']==10)
						if(strlen($val['l'][0])) $pais = implode(",", $val['l']);
