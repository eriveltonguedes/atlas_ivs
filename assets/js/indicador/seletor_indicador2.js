...
function IndicatorSelector()
{
    ...
    var lazy_array;
    var multiYear = false;
    var divSeletor = $('<div class="divSeletor">').addClass("divCallOut");
    var listener = null;
    ...
    this.getValueIndicador = function () {
    ...
    this.startSelector = function (multiselect, id_element_context, _listener, orientation, multiYear_, _to_hide, _skipLimit)
    {
        ...
        html += '<div><div class="dimensao box dim"><h6 class="title_box">' + lang_mng.getString("seletor_dimensao") + '</h6><ul class="nav nav-list list_menu_indicador dim">' +
                ...
                '</ul></div>';
        if (multiselect == true) {
            html += '<div class="itens_selecionados box"><a class="close indicador" data-backdrop="false" data-dismiss="modal">&times;</a><h6 class="title_box">' + lang_mng.getString("seletor_selecionados") + '</h6><div class="quantidade-indicadores-selecionados"> (0) </div><ul class="nav nav-list list_menu_indicador"></ul></div></div>';
        }
        html += '</div>';
        /* html += '<div class="btn_select" style="display:block">';
        ...
        var btnFooterModal = '<div class="btn_select" style="display:block">';
        btnFooterModal += '<div class="messages"></div>';
        btnFooterModal += '<div class="buttons">';
        btnFooterModal += '<button class="blue_button big_bt btn_ok" type="button" data-backdrop="false" data-dismiss="modal" style="float: right; font-size: 14px; height: 30px; padding: 5px 10px;">Ok</button>';
        btnFooterModal += '<button class="gray_button big_bt btn_clean" type="button" style="width: 175px; font-size: 14px; height: 30px; margin-left: 20px;">' + lang_mng.getString("limpar_selecionados") + '</button><div>';
        btnFooterModal += '</div>';
        $("#modal-seletor-indicadores").find(".modal-footer").html(btnFooterModal);
        $("#calloutIndicadores").on("click", '.selector_popover', function (e) {
            $('.popover-component').remove();
            $("#modal-seletor-indicadores").find(".modal-body").html(divSeletor);
            $(this_selector_element).find('.messages').html("");
            $(to_hide).find('.divCallOutLugares').removeClass('open');
            $(to_hide).find('.divCallOutLugares .popover').css('display', 'none');
            $("#modal-seletor-indicadores").modal();
    ...
    function startPopOver(multiselect)
    {
        ...
        if (load == false)
        {
            ...
            if (multiselect == true)
            {
                ...
                $(this_selector_element).on("click", '.btn_clean', function (e) {
                    ...
                    value_indicador_old = value_indicador.slice();
                    geral.setIndicadores(new Array());
                    fillSelectedItens();
                    setQuantitySelectedInd();
                });

                $(this_selector_element).on("click", '.btn_ok', function (e) {
                    $(this_selector_element).find('.divCallOut .popover').toggle();
                    var espacialidade = seletor.getSelecteds();
                    if(espacialidade.length === 0 || value_indicador.length === 0){
                        $('#imgTab2,#imgTab3,#botao-agregacao-tabela,#imgTab6').addClass('disabled');    
                    }
                    else{
                        // libera menu consulta
                        $('#imgTab2,#imgTab3,#botao-agregacao-tabela,#imgTab6').removeClass('disabled');
                    ...
                    if (value_indicador.length === 0) { //destroy a tabela e esconde
                        
                        // adiciona menu consulta

                        
                        
                        /*var table = tableComponent.getTable()
                         tableComponent.hideTable();
                         table.destroy();*/
                        TableComponent.destroy(TableComponent.getSettings())
                    }
                    else {
                        //cria a tabela e mostra
                        geral.setIndicadores(value_indicador);

                        

                        if (typeof TableComponent === "undefined") { //código antigo
                            $(this_selector_element).find('.divCallOut .popover').toggle();
                            dispatchListener();
                        }
                        else {//código novo
                    ...
                    if (seletor.fs.selectedItensIsEmpty() && geral.getIndicadores().length === 0)
                        AtlasBrasil.messages({body: "<strong>Próximo passo: Selecione uma espacialidade.</strong>"})
                    else
                        $(".general-alert").hide();
                });
            }
    ...
    {
        $(this_selector_element).on("click", ".itens_selecionados ul li .indicador_ano span", function ()
        {
    ...
    function fillData(data, multiselect)
    {
        ...
        $(this_selector_element).on("click", '.dimensao ul li', function (e) {
    ...
    function filtro_tema(value, multiselect)
    {
        ...
        $(this_selector_element).on("click", '.tema ul li', function (e)
        {
...
//        }
    ...
    function setQuantitySelectedInd() {

        $('.quantidade-indicadores-selecionados').html('(' + tab_indc.getValueIndicador().length + ')');
    }
    ...
    function listenerClickIndicador(multiselect)
    {
        ...
        {
            $(this_selector_element).on("click", '.indicador ul li', function (e) {
        ...
        {
            $(this_selector_element).on("click", '.indicador ul li a', function (e) {
